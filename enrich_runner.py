...
def main() -> int:
    ...
    ap.add_argument("--backfill-site", action="store_true",
                    help="add photos + opening hours to cached records via their known website (no search)")
    args = ap.parse_args()
    ...
    # Site backfill: add photos + opening hours to already-enriched records that lack them,
    # reusing each record's known website (no web search -> no rate-limit risk).
    if args.backfill_site:
        targets = [k for k, r in cache.items()
                   if r.get("website") and (not r.get("practice_photo") or not r.get("opening_hours"))]
        print(f"[backfill] {len(targets)} cached practices (photos + opening hours)")
        for i, k in enumerate(targets, 1):
            try:
                if enrich.backfill_site(cache[k]):
                    r = cache[k]
                    flags = "".join(["🌙" if r.get("has_evening") else "", "📅" if r.get("has_weekend") else ""])
                    print(f"  [{i}/{len(targets)}] {r.get('practice','')[:36]} {flags} {r.get('opening_hours') or ''}")
            except Exception as exc:  # noqa: BLE001
                print(f"  [{i}/{len(targets)}] FAIL {k}: {exc}")
